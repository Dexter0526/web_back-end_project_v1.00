...
user_hit = data.frame(row.names = userid, movie, variety, news, docu)
user_hit
test = data.frame(movie, variety, news, docu)
test

call_info <- function(userid, user_hit) {

  subset(user_hit, pname==userid)
...
rcorr(as.matrix(user_hit), type = "spearman")

pca <- princomp(user_hit, cor = T)
summary(pca)
biplot(pca)

d <- dist(user_hit, method = "euclidean")
...
d <- dist(user_hit$movie, method = "euclidean")
